...
#sql stmts
#div0_sql = "insert into raw_div0(id, div0_type) values (%s, %s)"
#div1_sql = "insert into raw_div1(id, div1_type) values (%s, %s)"
#interp_sql = "insert into raw_interp(inst, interp_type, value) values (%s, %s, %s)"
#join_sql = "insert into raw_join(result, targets) values (%s, %s)"
#rs_sql = "insert into raw_rs(inst, rs_type, value) values (%s, %s, %s)"
...
def connect(in_sql, in_data, in_arg):
	...
	# execute a statement
        try:
            if in_arg in ['two']:
                argument_string = ",".join("('%s', '%s')" % (x, y) for (x, y) in in_data)
            else:
